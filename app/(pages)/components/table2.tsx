...
export default function ReusableTable<T extends Record<string, any>>({
  ...
  customForm,
  customEditForm,
  openForm: externalOpenForm,
  onOpenFormChange,
}: ReusableTableProps<T>) {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(defaultRowsPerPage);
  const [searchTerm, setSearchTerm] = useState("");
  const [internalOpenForm, setInternalOpenForm] = useState(false);
  const [editingRow, setEditingRow] = useState<T | null>(null);
  const [deleteRow, setDeleteRow] = useState<T | null>(null);
  const [formValues, setFormValues] = useState<Partial<T>>({});

  // Use external or internal state for form visibility
  const openForm = externalOpenForm !== undefined ? externalOpenForm : internalOpenForm;
  const setOpenForm = onOpenFormChange || setInternalOpenForm;
  ...
  return (
    <Paper className="p-4">
      ...
      <Dialog 
        ...
        maxWidth={customForm ? "md" : "sm"} 
        fullWidth
        scroll="paper"
      >
        <DialogTitle>
          {editingRow ? "Edit Item" : "Add Item"}
        </DialogTitle>
        <DialogContent dividers>
          {customForm && !editingRow ? (
            // Render custom form for adding
            React.cloneElement(customForm as React.ReactElement, {
              onSubmit: (data: any) => {
                // Transform data if needed and call onAdd
                handleSave();
              }
            })
          ) : customEditForm && editingRow ? (
            // Render custom form for editing
            customEditForm(editingRow, (updatedItem: T) => {
              onEdit?.(updatedItem);
              setOpenForm(false);
            })
          ) : (
